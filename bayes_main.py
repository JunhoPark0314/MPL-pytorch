...
def train_loop(args, labeled_loader, unlabeled_loader, test_loader,
    ...
    for step in range(args.start_step, args.total_steps):
        ...
        with amp.autocast(enabled=args.amp):
            ...
            t_log_prob = Normal(loc=t_mu, scale=t_sigma).log_prob(sampled_teacher).mean()

            #t_loss_mpl = dot_product * F.cross_entropy(t_logits_us, hard_pseudo_label)
            t_loss_mpl = dot_product * t_log_prob
            t_loss = t_loss_uda + t_loss_mpl + t_loss_kl / len(t_logits_l)
