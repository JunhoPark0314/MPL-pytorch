...
from bayes_model import BayesWideResNet
...
parser.add_argument("--teacher-pretrained", type=str, default='', help="checkpoint for teacher model")
parser.add_argument("--max-history", type=int, default=1, help="maximum history to utilize")
...
def flatten(d, parent_key='', sep='_'):
    param_re = re.compile("|".join(["kernel", "weight", "bias"]))
    if any([param_re.search(str(k)) for k in d.keys()]):
        return dict([(parent_key,d)])
    items = []
    for k, v in d.items():
...
def _inner_compute_sampling(weight, grad, teacher_param, log_prob_sum):
    tta_sum = 0
    param_len = 0

    for layer in list(weight.keys()):
        curr_weight_dict = weight[layer]
        curr_grad_dict = grad[layer]
        curr_param = teacher_param[layer]
        curr_log_prob_sum = log_prob_sum[layer]

        for k in list(curr_weight_dict.keys()):
            curr_weight = curr_weight_dict[k]
            curr_grad = curr_grad_dict[k].cuda()
            kl_grad = curr_grad_dict["kl_{}".format(k)].cuda()
            mu_param = curr_param["mu_{}".format(k)]
            scale_param = torch.log1p(torch.exp(curr_param["rho_{}".format(k)]))
            pred_noise = (curr_weight.cuda() - mu_param.data) / scale_param.data 
            curr_dist = Normal(loc=mu_param.data, scale=scale_param.data)
            acc_coef = curr_dist.log_prob(curr_weight.cuda()).exp() / curr_log_prob_sum[k]
            tta_sum += (acc_coef * (curr_grad * (mu_param + pred_noise * scale_param) + kl_grad * pred_noise * scale_param)).sum()
            param_len += mu_param.numel() + scale_param.numel()

    return tta_sum

def _compute_log_prob(weight, teacher_param):
    log_prob_dict = {}
    for layer in list(weight.keys()):
        curr_weight_dict = weight[layer]
        curr_param = teacher_param[layer]
        ...
        for k in list(curr_weight_dict.keys()):
            curr_weight = curr_weight_dict[k]
            mu_param = curr_param["mu_{}".format(k)]
            rho_param = curr_param["rho_{}".format(k)]
            curr_dist = Normal(loc=mu_param.data, scale=torch.log1p(torch.exp(rho_param.data)))
            curr_log_prob[k] = curr_dist.log_prob(curr_weight.cuda()).exp()

        log_prob_dict[layer] = curr_log_prob

    return log_prob_dict
            
def compute_sampling_loss(grad_history, curr_log, step, max_history):
    reward = 0

    """
    for k, v in flatten_sampled_teacher.items():
        curr_delta = [k.split('_')[1] for k in v.keys() if 'delta' in k]
        for delta_key in curr_delta:
            teacher_dist[str.join('_',[k,delta_key])]=Normal(loc=v["mu_{}".format(delta_key)],
                    scale=torch.log1p(torch.exp(v["rho_{}".format(delta_key)])))
    """
    hs = min(step, max_history)
    rho_to_sig = lambda x: torch.log1p(torch.exp(x))
    mu_param = curr_log["mu_param"]
    scale_param = rho_to_sig(curr_log["rho_param"])

    curr_dist = Normal(loc=mu_param, scale=scale_param)
    history_weight = grad_history["weight"][:hs+1].cuda()
    history_log_prob = curr_dist.log_prob(history_weight).exp()
    history_grad = grad_history["weight_grad"][:hs+1].cuda()
    history_kl_grad = grad_history["kl_grad"][:hs+1].cuda()

    pred_noise = ((history_weight - mu_param) / scale_param).data
    acc_coef = (history_log_prob / history_log_prob.sum(dim=0))
    acc_loss =  acc_coef * (history_grad * (mu_param + pred_noise * scale_param) + history_kl_grad * pred_noise * scale_param) 

    return acc_loss.sum()

def vectorize(weight_history, grad_history, teacher_param):
    weight_history = flatten(weight_history)
    grad_history = flatten(grad_history)
    teacher_param = flatten(teacher_param)

    vectorize_weight = []
    vectorize_weight_grad = []
    vectorize_kl_grad = []
    vectorize_mu_param = []
    vectorize_rho_param = []
    for per_layer_weight, per_layer_grad, per_layer_teacher_param in zip(weight_history.values(), grad_history.values(), teacher_param.values()):
        for k in list(per_layer_weight.keys()):
            vectorize_weight.append(per_layer_weight[k].flatten())
            vectorize_weight_grad.append(per_layer_grad[k].flatten())
            vectorize_kl_grad.append(per_layer_grad["kl_{}".format(k)].flatten())
            vectorize_mu_param.append(per_layer_teacher_param["mu_{}".format(k)].view(-1).contiguous())
            vectorize_rho_param.append(per_layer_teacher_param["rho_{}".format(k)].view(-1).contiguous())

    vectorize_weight = torch.cat(vectorize_weight)
    vectorize_weight_grad = torch.cat(vectorize_weight_grad)
    vectorize_kl_grad = torch.cat(vectorize_kl_grad)
    vectorize_mu_param = torch.cat(vectorize_mu_param).contiguous()
    vectorize_rho_param = torch.cat(vectorize_rho_param).contiguous()

    vectorize_history = {
        "weight": vectorize_weight,
        "weight_grad": vectorize_weight_grad,
        "kl_grad": vectorize_kl_grad,
        "mu_param": vectorize_mu_param,
        "rho_param": vectorize_rho_param
    }

    return vectorize_history
        
def extract_grad(sampled_teacher):
    """
    extract gradient per sampled weight and copy weight in dictionary
    """
    weight_history_dict = {}
    grad_history_dict = {}
    param_dict = {}
    ...
    delta_key = []
    for k, v in sampled_teacher.items():
        if type(v) is dict:
            k_weight_dict, k_grad_dict, k_param_dict  = extract_grad(v)
            weight_history_dict[k] = k_weight_dict
            grad_history_dict[k] = k_grad_dict
            param_dict[k] = k_param_dict
        else:
            weight_history_dict[k] = v.detach().cpu()
            grad_history_dict[k] = v.grad.detach().cpu()
            if 'delta' in k:
                delta_key.append(k)
            else:
                param_dict[k] = v
    ...
    if len(delta_key):
        ...
        sample_param = {}
        for k in delta_key:
            origin_key = k.split('_')[1]
            mu_key = "mu_{}".format(origin_key)
            rho_key = "rho_{}".format(origin_key)
            delta_key = "delta_{}".format(origin_key)
            sample_weight[origin_key] = weight_history_dict[delta_key] + weight_history_dict[mu_key]
            sample_grad[origin_key] = grad_history_dict[mu_key]
            sample_grad["kl_{}".format(origin_key)] = grad_history_dict[delta_key] - grad_history_dict[mu_key]
            sample_param[origin_key] = {mu_key: param_dict[mu_key], rho_key:param_dict[rho_key]}
        
        weight_history_dict = sample_weight
        grad_history_dict = sample_grad

    return weight_history_dict, grad_history_dict, param_dict
...
def train_loop(args, labeled_loader, unlabeled_loader, test_loader,
    ...
    for step in range(args.start_step, args.total_steps):
        ...
        mpl_mask = float(step < args.student_wait_steps)
        with amp.autocast(enabled=args.amp):
            batch_size = images_l.shape[0]
            t_images = torch.cat((images_l, images_uw, images_us))
            t_logits, kl_t, sampled_teacher = teacher_model(t_images)
            ...
            t_loss_uda = t_loss_l + weight_u * t_loss_u * mpl_mask
        ...
        # TODO: change teacher loss part with accumulated dot-product loss
        with amp.autocast(enabled=args.amp):
            with torch.no_grad():
                s_logits_l = student_model(images_l)
            s_loss_l_new = F.cross_entropy(s_logits_l.detach(), targets)
            dot_product = s_loss_l_new - s_loss_l_old
            dot_product_record = dot_product.detach().clone()
            ...
            t_loss = t_loss_uda + t_loss_mpl * mpl_mask + kl_t / args.num_labeled

        if step < args.student_wait_steps or args.max_history == 0:
            t_scaler.scale(t_loss).backward()
            t_scaler.step(t_optimizer)
            t_scaler.update()
        else:

            with teacher_model.no_sync():
                t_loss.backward()
                weight_history, grad_history, teacher_param = extract_grad(sampled_teacher)
                vectorize_history = vectorize(weight_history, grad_history, teacher_param)

                if acc_history["weight"] is None:
                    acc_history["weight"] = torch.empty((args.max_history,vectorize_history["weight"].shape[0])).cuda()
                    acc_history["weight_grad"] = torch.empty((args.max_history,vectorize_history["weight_grad"].shape[0])).cuda()
                    acc_history["kl_grad"] = torch.empty((args.max_history,vectorize_history["kl_grad"].shape[0])).cuda()
                
                acc_history["weight"][step % args.max_history] = vectorize_history["weight"].cuda()
                acc_history["weight_grad"][step % args.max_history] = vectorize_history["weight_grad"].cuda()
                acc_history["kl_grad"][step % args.max_history] = vectorize_history["kl_grad"].cuda()

                teacher_model.zero_grad()

            temporal_reward_acc = compute_sampling_loss(acc_history, vectorize_history, step, args.max_history)

            temporal_reward_acc.backward()
        ...
        if args.world_size > 1:
            ...
            dot_product = reduce_tensor(dot_product_record.detach(), args.world_size)
        ...
        dot_product_mean.update(dot_product.item())
        ...
        pbar.set_description(
            ...
            f"T_Loss: {t_losses.avg:.2f}. Mask: {mean_mask.avg:.2f}. T_Loss_l: {t_losses_l.avg:.2f}. T_Loss_u: {t_losses_u.avg:.2f}. T_Loss_mpl: {t_losses_mpl.avg:.2f}. DP:{dot_product_mean.avg:.2f}")
        pbar.update()
...
def main():
    ...
    # TODO: change teacher model with weight sampling network
    teacher_model = BayesWideResNet(num_classes=args.num_classes,
                               depth=depth,
    ...
    if args.teacher_pretrained:
        loc = f'cuda:{args.gpu}'
        checkpoint = torch.load(args.teacher_pretrained, map_location=loc)
        logger.info("=>Teacher model best_prec1: {}, epoch: {}".format(checkpoint["best_prec1"], checkpoint["epoch"]))
