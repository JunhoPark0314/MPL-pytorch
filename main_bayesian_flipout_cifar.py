...
def main():
    ...
    depth = 28
    widen_factor= 2
    teacher_model = BayesWideResNet(num_classes=10,
                               depth=depth,
                               widen_factor=widen_factor,
                               dropout=0,
                               dense_dropout=0.2)

    model = torch.nn.DataParallel(teacher_model)
...
def train(args,
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        #ELBO loss
        loss = cross_entropy_loss + scaled_kl.mean()
        '''
        ...
        if tb_writer is not None:
            ...
            tb_writer.add_scalar('train/kl_div', scaled_kl.mean().item(), epoch)
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            if tb_writer is not None:
                tb_writer.add_scalar('val/cross_entropy_loss',
                                     cross_entropy_loss.item(), epoch)
                tb_writer.add_scalar('val/kl_div', scaled_kl.mean().item(), epoch)
